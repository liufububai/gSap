...
inline
void createTorus(float M, float N, 
			   int4* quads, float4* vtx)
{
	int i, j, k, numc, numt, id, id1,id2,id3;
	...
	for(k=0; k<num; k++)
	{
		for(i = 0; i < numc; i++)
			for(j = 0; j < numt; j++)
			{
				s = i;// % numc;
				t = j;// % numt;
				x = (M + N*cos(s*twopi/numc)) * cos(t*twopi/numt);// + ((k%nRow)-nRow/2)*spacing;
				y = (M + N*cos(s*twopi/numc)) * sin(t*twopi/numt);// + ((k)/(nRow*nRow)+1)*spacing;
				z = N * sin(s*twopi/numc);// + (((k/nRow)%nRow)-nRow/2)*spacing;
				vtx[k*150 + i*numt + j] = make_float4(x, y, z, 0);
			}
		...
		for(i = 0; i < numc; i++)
			...
			{
				id = i*numt + j;
				id1 = (id+numt)%(numc*numt);
				id3 = id+1 < (i+1)*numt ? id+1 : id+1 - numt;
				if(id+numt < numc*numt)
					id2 = id+numt+1 < (i+2)*numt ? id+numt+1 : id+numt+1 - numt;
				else
					id2 = id1+1 < numt ? id1+1: id1+1 - numt;
				quads[k*150 + id] = make_int4(id+k*150, id1+k*150, id2+k*150, id3+k*150);
			}
	}
}
