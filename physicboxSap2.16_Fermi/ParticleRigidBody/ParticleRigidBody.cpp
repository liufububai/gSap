...
m_maxLinVelocity( VEL_MAX  )
{
	...
	m_springMultip = 0.5f;
	m_dampingMultip = 0.25f;
	pbo_position = 1; //Fuchang Liu
	pbo_quater = 2; //Fuchang Liu
	texSize = 64; //Fuchang Liu
}
...
ParticleRigidBody::~ParticleRigidBody()
{
	...
	cudaGLRegisterBufferObject(pbo_position); //Fuchang Liu
	cudaGLRegisterBufferObject(pbo_quater); //Fuchang Liu
}
...
void ParticleRigidBody::setRbData(RigidBodyData& rbData)
{
	...
	//Fuchang Liu
	glGenBuffersARB(1, &pbo_position);
    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER, pbo_position); //Fuchang Liu
    glBufferDataARB(GL_PIXEL_UNPACK_BUFFER, 4*texSize*texSize*sizeof(float), 0, GL_STREAM_DRAW_ARB);  //Fuchang Liu
	glBindBufferARB(GL_PIXEL_UNPACK_BUFFER, 0);
    glGenBuffersARB(1, &pbo_quater);
    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER, pbo_quater); //Fuchang Liu
    glBufferDataARB(GL_PIXEL_UNPACK_BUFFER, 4*texSize*texSize*sizeof(float), 0, GL_STREAM_DRAW_ARB);  //Fuchang Liu
    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER, 0);
	// register this buffer object with CUDA
	cudaGLRegisterBufferObject(pbo_position);
	cudaGLRegisterBufferObject(pbo_quater);
	//Fuchang Liu
...
void ParticleRigidBody::setParticleData(float4* pos, float* mass, int numElements)
{
	...
	TransferUtils::copyHostToDevice(m_particleMass, mass, sizeof(float)*numElements);
}
